...
export default class LifeCycle extends Component {
    ...
    static getDerivedStateFromProps(nextProps, preState) {
        console.log(`[MOUNT02][UPDATE01]: getDerivedStateFromProps(nextProps=${nextProps.color}, preState=${preState.color})`);
        return nextProps.color !== preState.color ? {color: nextProps.color} : null
    }

    /**
     * state 변경 되었을 때, re-rendering 여부를 결정한다.
    ...
    shouldComponentUpdate(nextProps, nextState) {
        console.log(`[UPDATE02]: shouddComponentUpdate(nextProps=${nextProps.color}, nextState=${nextState.color})`);
        return true;
    }
    ...
    /**
     * render 메소드 호출 직 후, DOM에 변화를 반영하기 직전에 호출[react V16.3]
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        ...
        console.log(`[UPDATE05]: getSnapshotBeforeUpdate(prevProps=${prevProps.color}, prevState=${prevState.color}, snapshot=${snapshot})`);
    }
